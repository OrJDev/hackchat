...
import { QueryBoundary, RenderUserImage } from "~/components";
          ...
          <>
            ...
            <Meta
              ...
              content={
                data && data?.name && data?.image
                  ? `${getUrl()}/api/contact-image?name=${data?.name}&image=${
                      data?.image
                    }`
            ...
            <main class="flex flex-col gap-5 w-full h-full items-center">
              <div class="w-full flex gap-2 items-center justify-center px-3 py-1 sm:py-3 sm:px-12">
                <RenderUserImage sm img={data?.image} name={data?.name} />
                <div class="text-offwhite text-sm sm:text-xl font-bold">
                  <strong class="text-gray-400">{data?.name}</strong>{" "}
              ...
              >
                    ...
                    onClick={() =>
                      void wrapWithTry(() =>
                        acceptLink.mutateAsync({ id: params.id }).then(() => {
                          toast.success(
                            `${data?.name} Is Now In Your Contacts`
                          );
                          setContacts((prev) => [
                            {
                              img: data?.image,
                ...
                <p class="text-offwhite font-medium max-w-[300px] sm:max-w-[400px] text-center">
                  By accepting {data?.name}'s request, he will be able to send
                  you messages, you can also ignore this request for now and
                  accept it later.
                </p>
              </Show>
            </main>
